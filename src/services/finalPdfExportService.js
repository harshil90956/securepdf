...
import { getGhostscriptBin } from './ghostscriptBin.js';
...
export async function materializeFinalPdfExportKey(documentId) {
  ...
  try {
    await s3.send(new HeadObjectCommand({ Bucket: process.env.S3_BUCKET, Key: exportKey }));
    if (redis) {
      try {
