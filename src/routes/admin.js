...
router.get('/vector-jobs/:jobId/debug', authMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    const jobDoc = await VectorPrintJob.findById(jobId).exec().catch(() => null);
    if (!jobDoc) {
    ...
    return res.json({
      jobId: jobDoc._id,
      traceId: jobDoc.traceId || null,
      status: jobDoc.status,
      lifecycleHistory: Array.isArray(jobDoc.lifecycleHistory) ? jobDoc.lifecycleHistory : [],
      createdAt: jobDoc.createdAt,
      updatedAt: jobDoc.updatedAt,
      batchStartedAt: jobDoc.batchStartedAt || null,
      batchFinishedAt: jobDoc.batchFinishedAt || null,
      mergeStartedAt: jobDoc.mergeStartedAt || null,
      mergeFinishedAt: jobDoc.mergeFinishedAt || null,
      errorCode: jobDoc.errorCode || null,
      errorAt: jobDoc.errorAt || null,
    });
  } catch (err) {
    return res.status(500).json({ message: 'Internal server error' });
  }
